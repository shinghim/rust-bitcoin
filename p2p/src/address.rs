...
impl<'a> Arbitrary<'a> for Address {
    fn arbitrary(u: &mut Unstructured<'a>) -> arbitrary::Result<Self> {
        Ok(Address{
            services: ServiceFlags::arbitrary(u)?,
            address: u.arbitrary()?,
...
impl<'a> Arbitrary<'a> for AddrV2 {
    fn arbitrary(u: &mut Unstructured<'a>) -> arbitrary::Result<Self> {
        match u.int_in_range(0..=5)? {
            0 => Ok(AddrV2::Ipv4(Ipv4Addr::from_bits(u.arbitrary()?))),
            1 => Ok(AddrV2::Ipv6(Ipv6Addr::from_bits(u.arbitrary()?))),
            2 => Ok(AddrV2::TorV3(u.arbitrary()?)),
            3 => Ok(AddrV2::I2p(u.arbitrary()?)),
            4 => Ok(AddrV2::Cjdns(Ipv6Addr::from_bits(u.arbitrary()?))),
